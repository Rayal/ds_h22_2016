...
def conn_req (server_obj, mqtt, args):
    ...
    if server_obj.new_client(client, nick):
        mqtt_publish(mqtt, "/".join((DEFAULT_ROOT_TOPIC, SERVER, server_obj.self, client)), " ".join((CONN_REQ, YEA)))
    else:
        mqtt_publish(mqtt, "/".join((DEFAULT_ROOT_TOPIC, SERVER, server_obj.self, client)), " ".join((CONN_REQ, NAY)))

def game_list_req(server_obj, mqtt, args):
    client = args[0]
    game_list = server_obj.get_game_list()
    mqtt_publish(mqtt, "/".join((DEFAULT_ROOT_TOPIC, SERVER, server_obj.self, client)), " ".join((GAME_LIST, game_list)))
...
def join_game(server_obj, mqtt, args):
    ...
    if response == 0:
        ...
        mqtt_publish(mqtt, "/".join((DEFAULT_ROOT_TOPIC, SERVER, server_obj.self, client)), ' '.join((JOIN_GAME, NAY)))
    elif response == -1:
        # Game not found
        ...
        mqtt_publish(mqtt, "/".join((DEFAULT_ROOT_TOPIC, SERVER, server_obj.self, client)), ' '.join((JOIN_GAME, str(response))))
...
def create_game(server_obj, mqtt, args):
    ...
    if response == 0:
        ...
        mqtt_publish(mqtt, "/".join((DEFAULT_ROOT_TOPIC, SERVER, server_obj.self, client)), ' '.join((CREATE_GAME, NAY)))
    elif response == -1:
