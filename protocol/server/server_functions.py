...
def game_list_req(server_obj, mqtt, args):
    ...
    mqtt_publish(mqtt, "/".join((DEFAULT_ROOT_TOPIC, SERVER, SELF, client)), " ".join((GAME_LIST, game_list)))
...
def join_game(server_obj, mqtt, args):
    ...
    if response == 0:
        ...
        mqtt_publish(mqtt, "/".join((DEFAULT_ROOT_TOPIC, SERVER, SELF, client)), str(response))
